...
gm_s16 levelpicnum[10] = {10,10,10,16,16,16,16,12,12,16};//√øπÿø…—°‘ÒµƒÕº∆¨ ˝;
...
void initMemoryBoxplay(MemoryBoxplay * myself,Rect windows_Rect,DF_PictureLibrary * picLib,gm_s16 repaintMax)
{
	...
	myself->picw=50;   //√ø∏ˆøÈµƒøÌ∏ﬂ
    myself->pich=50;
    myself->x0=20 ;  //◊Ó◊Û…œøÈµƒŒª÷√
    myself->y0=50;
	myself->sourcetime=0;
	MemoryBox_Loading_UpdateStep();

	myself->ingame=false;// «∑Ò’˝‘⁄”Œœ∑£¨»√ ±º‰‘À––
    myself->soundopen=true;//”Œœ∑…˘“Ùø™π
    MemoryBox_Loading_UpdateStep();
...
void selectpic(MemoryBoxplay * myself,gm_s16 m)//¥”÷¡…Ÿ10∏ˆÕº∆¨¿Ô»°10∏ˆÕº∆¨
 {
 	 gm_s16 i;
	 for(i=0;i<16;i++)
		  pictemp[i]=false;
 ...
 //√øæ÷≥ı ºªØ
  void MemoryBoxinit(MemoryBoxplay * myself) 
  {
        ...
        if(level==1)
            ...
            myself->sourcetime+=70;//√øπ˝“ªπÿ ±º‰∂ºº”55
        if(gamestage==5)//»Áπ˚ «¡∑œ∞ƒ£ Ω£¨ ±º‰≥ı ºªØŒ™0£¨
            myself->sourcetime=0;
	        myself->temptime=0;
	        myself->guangbiaox=0;//π‚±ÍŒª÷√∑≈‘⁄◊Ó◊Û…œ
	        myself->guangbiaoy=0;
        ...
        for( i=0;i<20;i++) {
            if(i%2==0) {
                ...
                while(selecttemp[myself->rand]==true)//±£÷§»°π˝µƒÕº∆¨≤ª±ª‘Ÿ»°
                    myself->rand=Random(0,9);
                selecttemp[myself->rand]=true;}
//…œ√Ê5––‘⁄10’≈Õº∆¨¿Ô»°“ª’≈µƒπ˝≥Ã
            clip=Random(0,19);
            while(randtemp[clip/5][clip%5])//±£÷§“—æ≠±ª∏≥÷µµƒøÈ≤ª±ª÷ÿ∏¥∏≥÷µ
                clip=Random(0,19);
            randtemp[clip/5][clip%5]=true;
            game[clip/5][clip%5]=pic[myself->rand];
//…œ√Ê5––∞—»°≥ˆµƒÕº∆¨∏¯“ª∏ˆÀÊª˙µƒøÈ
        }
        myself->zhuangtai=0;
    }
...
{
	char buff[30];
	sprintf((char*)buff,"ƒ—∂»%d",level);
	getManagerPtr()->DrawString(buff,40,15,8,0xffff00);
	if (gamestage == 1)
		getManagerPtr()->DrawString(" £”‡ ±º‰",40,105,8,0xffff00);
	else
		getManagerPtr()->DrawString(" π”√ ±º‰",40,105,8,0xffff00);
...
void MemoryBox_Paint_Game(MemoryBoxplay * myself) 
{
	...
	if(gamestage==1)//”Œœ∑ƒ£ Ωœ¬ ±º‰µ›ºı
	{
		...
		if(myself->sourcetime==myself->starttime-1&&myself->temptime == myself->tempstarttime) 
		{
			if(myself->sourcetime>0 && myself->opennum<10)
				myself->zhuangtai=0;
			if(game[myself->clipx1][myself->clipy1]==game[myself->clipx2][myself->clipy2])//»Áπ˚—°≥ˆµƒ¡Ω∏ˆÕº∆¨œ‡Õ¨£¨‘Úœ‘ æ∂‘∫≈
			{
		...
		if(myself->sourcetime==0)// ±º‰µΩ£¨”Œœ∑Ω· ¯
		{
			myself->ingame=false;
			myself->gameoverl=true;
			myself->zhuangtai=4;
		}
		if(myself->opennum==10)//»Áπ˚»´≤øµƒøÈ∂º’“µΩ£¨‘Úœ‘ æπ˝πÿªÚÕ®πÿ
		{
			if(level==10) 
			{
				if(myself->sourcetime>550-memoryBoxHighTimeValue[9]) //∏¸–¬º«¬º
				{
					myself->pojilu=true;
	...
	else if(gamestage==5)//¡∑œ∞ƒ£ Ωœ¬ ±º‰µ›‘ˆ
	{
		if(myself->temptime==0&&myself->ingame)
			myself->sourcetime++;
		...
		if(myself->sourcetime==myself->starttime+1&&myself->temptime==myself->tempstarttime) 
		{
			if(myself->opennum<10)
				myself->zhuangtai=0;
			if(game[myself->clipx1][myself->clipy1]==game[myself->clipx2][myself->clipy2]&&!(myself->clipx1==myself->clipx2&&myself->clipy1==myself->clipy2))//»Áπ˚—°≥ˆµƒ¡Ω∏ˆÕº∆¨œ‡Õ¨£¨‘Úœ‘ æ∂‘∫≈
			{
				clipstage[myself->clipx1][myself->clipy1]=2;
				clipstage[myself->clipx2][myself->clipy2]=2;
				if(myself->opennum<10) 
		...
		if(myself->opennum==10)//»Áπ˚»´≤øµƒøÈ∂º’“µΩ£¨‘Úœ‘ æ≥…π¶
		{
			myself->ingame=false;
			myself->zhuangtai=3;
			if(myself->sourcetime<memoryBoxHighTimeValue[level-1]) //∏¸–¬º«¬º
			{
				myself->pojilu=true;
	...
	for(i=0;i<4;i++)
		for(j=0;j<5;j++) 
		{
			...
			if(clipstage[i][j]==2)//œ‡Õ¨µƒøÈ’“µΩ∫Ûœ‘ æ∂‘∫≈
			{  
...
{
	   if(myself->zhuangtai==0&&clipstage[myself->guangbiaox][myself->guangbiaoy]==0)//√ª”–œ‘ æøÈ∑≠ø™µƒ ±∫Ú
	   {
	   ...
	   if(myself->zhuangtai==1&&clipstage[myself->guangbiaox][myself->guangbiaoy]==0)//”–“ª∏ˆøÈ±ª∑≠ø™µƒ ±∫Ú
	   {
	   	clipstage[myself->guangbiaox][myself->guangbiaoy]=1;
		myself->clipx2=myself->guangbiaox;
		myself->clipy2=myself->guangbiaoy;
		myself->starttime=myself->sourcetime;//¡Ω∏ˆøÈ±ª∑≠ø™∫Ûø™ ºº«¬º ±º‰£¨1√Î∫Û≤≈ø…“‘—°‘Ò
...
{
	gm_s16 px,py,i,j;
	if(getManagerPtr()->IsKeyDown(MF_KEY_RIGHTSOFT))//∞¥”“»Ìº¸∑µªÿ÷˜≤Àµ•
	{
		gamestage=0;
		level=1;
		myself->sourcetime=0;
	...
	{
		case 1:
		case 5://”Œœ∑ƒ£ Ωº∞¡∑œ∞ƒ£ Ω
		if(myself->ingame)//”Œœ∑π˝≥Ã÷–
		{
			if(getManagerPtr()->IsKeyDown(MF_KEY_2 | MF_KEY_UP))//∞¥…œœ¬◊Û”““∆∂Øπ‚±Í
			{
			...
			if(getManagerPtr()->IsKeyDown(MF_KEY_5 | MF_KEY_MID))//∞¥ø’∏Òº¸—°‘ÒøÈ
				MemoryBox_Select_Clip(myself);
			} 
			else if(getManagerPtr()->IsKeyDown(MF_KEY_MID))
			{
				if(gamestage==1&&myself->zhuangtai==3)//»Áπ˚”Œœ∑ƒ£ Ωœ¬≥…π¶£¨‘Úœ¬“ªπÿ
				{
					level++;
					if(level==11) 
					{
				...
				if((gamestage==1&&myself->zhuangtai==4)||(gamestage==5&&myself->zhuangtai==3))//»Áπ˚”Œœ∑ƒ£ Ωœ¬GAMEOVER ªÚ¡∑œ∞ƒ£ Ωœ¬≥…π¶£¨‘Ú∑µªÿ÷˜≤Àµ•
				{
					level=1;
					myself->sourcetime=0;
					gamestage=0;
...
void MemoryBox_Play_Windows_Update(void * param)
{
	...
	getManagerPtr()->DrawString("∞¥”“»Ìº¸ÕÀ≥ˆ",40,SCREEN_WIDTH/2-60,TOUCH_Y_RIGHT+10,0xffff00);
}
