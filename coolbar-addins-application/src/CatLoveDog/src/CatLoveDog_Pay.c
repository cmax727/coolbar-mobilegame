...
void initCatLoveDog_Pay(CatLoveDog_Pay * myself,Rect DF_Windows_Rect,DF_PictureLibrary * picLib)
{	
	initManager(&manager);
	initDFWindows(&myself->windows,DF_Windows_Rect,myself,myself,5);
	myself->windows.RenderCallBack=CatLoveDog_Pay_Windows_Render;
	myself->windows.UpdateCallBack=CatLoveDog_Pay_Windows_Update;
	...
	myself->textBox.SetText(&myself->textBox,"    本作是根据猫狗大战电影改编，游戏主人公是阿猫和阿狗，游戏生动可爱有趣,是一款玩法简单回味无穷的好游戏。\n \n    你可以使用短信(2元)或酷宝(28酷宝)支付，继续点播开始享受服务，返回则不扣费。\n  客服电话400-877-3373");
...
void CatLoveDogPayRender(CatLoveDog_Pay * myself)
{
...
{
	CatLoveDog_Pay * myself =(CatLoveDog_Pay *)param;
	CatLoveDogPayRender(myself);
}
...
//			sprintf((char*)enterStr,"点播后可享用%d天",extVmBillingManager->BILLING_GetValidDayByAppId(NETAPP_APP_CatLoveDog,NETAPP_BILLING_METHOD_ONCE_MONTH));
		...
		if(extOldGameManager->IsKeyDown(MF_KEY_LEFTSOFT) && extVmBillingManager->BILLING_GetRemainDay(NETAPP_APP_CatLoveDog,NETAPP_BILLING_METHOD_ONCE_MONTH) == 0)
		{
//			extVmBillingManager->BILLING_Pay3(NETAPP_APP_CatLoveDog,NETAPP_BILLING_METHOD_ONCE_MONTH,billingOnceCB,1,FALSE);
		}
		#ifdef GAME_176_220
...
//			if(CdRectPoint2(TOUCH_X_LEFT,TOUCH_Y_LEFT,TOUCH_X1_LEFT,TOUCH_Y1_LEFT,px,py) && extVmBillingManager->BILLING_GetRemainDay(NETAPP_APP_CatLoveDog,NETAPP_BILLING_METHOD_ONCE_MONTH) == 0)
//				extVmBillingManager->BILLING_Pay3(NETAPP_APP_CatLoveDog,NETAPP_BILLING_METHOD_ONCE_MONTH,billingOnceCB,1,FALSE);
		}
		#endif
	}
